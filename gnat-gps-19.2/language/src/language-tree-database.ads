...
package Language.Tree.Database is
   ...
   --  Type overridden in language_handlers.ads, which provides the necessary
   ...
   --  of time. The lock can be released either explicitly, by doing calls to
   --  Unlock, or implicitly, at object finalization. If two or more locks are
