...
class CLI(GPS.Process):
    ...
    def delete_logfile_dialog(filename):
        ...
        # first signal to log
        ask_overwrite = True
        for itid, sig in six.iteritems (QGEN_Module.signal_attributes):
            if sig.logged:
                ask_overwrite = False
                break
...
if not CLI.is_available():
    ...
    class QGEN_Module(modules.Module):
        ...
        def __clear(debugger):
            ...
            QGEN_Module.cancel_workflows()
            for id, sig in six.iteritems (QGEN_Module.signal_attributes):
                sig.reset()
        ...
        def on_diagram_changed(viewer, diag, clear=False):
            ...
            QGEN_Module.signal_attributes = {
                k: sig for k, sig in six.iteritems (QGEN_Module.signal_attributes)
