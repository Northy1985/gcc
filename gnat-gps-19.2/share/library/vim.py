...
class CharActionKeyMap(KeyMap):
    ...
    def get_action(self, keyval, vim_state):
        action = self.keymap.get(six.int2byte(keyval), (NoAction,))
...
class CharCharKeyMap(KeyMap):
    ...
    def get_action(self, keyval, vim_state):
        return create_action((CharAction, six.int2byte(keyval)), vim_state)
...
visual_actions = basic_actions.copy()
visual_actions ["d"] = (VisualDeletion,)
visual_actions ["D"] = (VisualDeletion,)
visual_actions ["y"] = (VisualYank,)
visual_actions ["Y"] = (VisualYank,)
