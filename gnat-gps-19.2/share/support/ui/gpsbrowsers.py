    ...
    def __load(self, data):
        ...
        if isinstance(data, six.binary_type):
            try:
                ...
                return
        if self.load_styles:
            for id, s in six.iteritems (data.get('styles', {})):
                self.styles.parse(s, None, id=id)

        for id, t in six.iteritems (data.get('templates', {})):
            self.templates[id] = t
...
class JSON_Diagram(B.Diagram):
    ...
    def merge_template(orig, template):
        ...
        """
        for field, value in six.iteritems (template):
...
class Diagram:
    ...
    def load_json(file, diagramFactory=None, load_styles=None):
        ...
        try:
            if isinstance(file, six.binary_type):
                file = open(file)
