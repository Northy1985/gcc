...
class AccessSpecifier(object):
    ...
    def __init__(self, value):
        if value >= len(AccessSpecifier._kinds):
            AccessSpecifier._kinds += (
                [None] * (value - len(AccessSpecifier._kinds) + 1)
            )
...
class _CXUnsavedFile(Structure):
    ...
    _fields_ = [
        ("name", c_char_p), ("contents", c_char_p), ('length', c_ulong)
    ]
...
class TranslationUnit(ClangObject):
    ...
    def codeComplete(self, path, line, column, unsaved_files=None,
        ...
        ptr = conf.lib.clang_codeCompleteAt(
            self, path, line, column, unsaved_files_array, len(unsaved_files),
            options
        )
        if ptr:
