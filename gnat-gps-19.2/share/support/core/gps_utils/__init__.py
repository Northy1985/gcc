...
def enum(**enums):
    ...
    name = 'Enum %s' % ', '.join(
        "%s=%s" % (k, v) for k, v in six.iteritems (enums))
...
def save_excursion(f, args, kwargs, undo_group=True):
    ...
    number of parameters (named or unnamed) can be passed in the usual way
...
def make_interactive(callback, category="General", filter="", menu="", key="",
    ...
    if isinstance(callback, six.class_types):  # Do we have a class ?
        def do():
